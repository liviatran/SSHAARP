...
PAL<-function(gdataset, motif){
  ...
  unique_AWM<-sapply(unique(AWMs$trimmed_allele), function(x) NULL)
  ...
  #sets last column generated by melt to NULL
  unique_AWM$L1<-NULL
  ...
  #merges PAF with solberg_DS data to a new variable, tbm_ds (to be mapped dataset)
  #which contains summed up allele frequencies, and relevant contin, complex, locus*allele, and coordinate information
  tbm_ds<-merge(PAF, solberg_DS[!duplicated(solberg_DS$popname),], by="popname")[,c("popname","contin", "complex", "latit", "longit", "allele_freq", "locus_allele")]
